...
def evalSymbReg(individual, input, output, nb_obj=1):
    ...
    # print("evalSymbReg ", individual, input, output)
    sqerrors = ((func(*in_) - out)**2 for in_, out in zip(input, output))
    #print(math.fsum(sqerrors) / len(input))
    return math.fsum(sqerrors) / len(input),
...
def main(nb_gen=100, mu=400, lambda_=400, res_dir='res', selection='elitist', problem='f1', state_dim=0, noise=0):

    random.seed()

    print("Number of generations: "+str(nb_gen))
    ngen = nb_gen
    print("Population size: "+str(mu))
    mu = mu
    print("Number of offspring to generate: "+str(lambda_))
    lambda_ = lambda_
    print("Selection scheme: "+str(selection))
    sel = selection
    if (sel == "nsga2"):
        nb_obj = 2
    else:
        nb_obj = 1
    print("Basename of the results dir: "+str(res_dir))
    name = res_dir

    print("State dim: "+str(state_dim)+" (ignored on most problems)")
    state_dim = state_dim
    ...
    if (problem == "f1"):
        ...
        def f(x):
            return x[0] * x[1] + math.cos(x[0]) + noise

        input_training = [random.sample(range(0, 10), 2) for i in range(30)]
        print(input_training)

        # à compléter
        output_training = [f(x) for x in input_training]

        input_testing = [random.sample(range(0, 10), 2) for i in range(30)]
        # à compléter
    ...
    pset.addTerminal(1)
    pset.addEphemeralConstant("rand101", lambda: ru())
    ...
    toolbox.register("evaluate", evalSymbReg,
                     input=input_training, output=output_training)
    ...
    for i, ind in enumerate(hof):
        ...
        print("Error on the training dataset: %f" % (evalSymbReg(
            ind, input_training, output_training, nb_obj=1)))
...
if (__name__ == "__main__"):
    ...
    parser = argparse.ArgumentParser(
        description='Launch symbolic regression run.')
